termux_step_massage() {
	if $TERMUX_SAFE_BUILD || ($TERMUX_PKG_PROOT && [ $TERMUX_PREFIX != /usr ]); then
		local old new mas
		mas="$TERMUX_PKG_MASSAGEDIR$prefix"
		old=$TERMUX_PREFIX
		$TERMUX_SAFE_BUILD && new=$prefix
		$TERMUX_PKG_PROOT && old=$TERMUX_PREFIX && new=/usr
		echo replacing prefix $old with $new in $mas folder
		pushd $mas
		set +e
