termux_step_massage() {
	if $TERMUX_ON_DEVICE_BUILD || $TERMUX_PKG_PROOT; then
		local new old
		$TERMUX_ON_DEVICE_BUILD && new=$TERMUX_PREFIX
		$TERMUX_PKG_PROOT && new=
		old=$TERMUX_PKG_MASSAGEDIR$TERMUX_PREFIX
		pushd "$TERMUX_PKG_MASSAGEDIR$TERMUX_PREFIX"
		set +e
