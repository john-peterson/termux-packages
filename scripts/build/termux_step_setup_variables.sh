termux_step_setup_variables() {
	...
	# -glibc suffix is considered for  removal from dependency tree in favour of search path order "gpkg packages" 
	...
	if $TERMUX_PKG_PROOT; then
		if [ "$TERMUX_PACKAGE_LIBRARY" = "glibc" ]; then
			termux_error_exit "renove --library flag from proot build"
		fi
